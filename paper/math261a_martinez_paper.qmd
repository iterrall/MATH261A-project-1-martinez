...
* **Normality of residuals** (4): departures from the diagonal line the Q–Q plot (@fig-pov-qq) and skew in the residual histogram (@fig-pov-histo) indicate deviations from normality for the residuals [@qqplotr; @Wickham2016].
...
#ggplot(to_analyze_df, aes(x = edu, y = pov)) + geom_point(alpha = 0.6, size = .4) +  geom_smooth(method = "lm", se = TRUE, color = "purple") +  annotate("label", x = -Inf, y = Inf, hjust = -0.1, vjust = 1.1, label = eq_math, parse = TRUE, label.size = 0.2, alpha = 0.5) +  labs(title = "Low-educational attainment vs. poverty rates", subtitle    = "Simple linear regression by census tract (2015–2019)", x        = "Adults without high school diploma (%)", y        = "Population under 2× FPL (%)", caption  = "Source: ACS 2015–2019; CalEnviroScreen 4.0")
```
```{r fig-pov-other}
#| fig-width: 5.25 
#| fig-height: 3.2 
...
#ue_text <- sprintf("y == %.3f + %.3f * x ~ \",\" ~ R^2 == %.3f", intercept2, slope2, r22) # create equation text


#scatter plot with regression
#ggplot(to_analyze_df_3vars, aes(x = edu, y = pov, color=unemp)) + geom_point(alpha = .6 , size = 0.4) + scale_color_viridis_c(name = "Unemployment (%)", option = "plasma")+geom_smooth(method = "lm", se = TRUE, color = "black") + annotate("label", x = -Inf, y = Inf, hjust = -0.1, vjust = 1.1, label = eq_math, parse = TRUE, label.size = 0.2, alpha = 0.5) + labs(title = "Education vs. poverty, colored by unemployment", subtitle    = "Simple linear regression by census tract (2015–2019)", x = "Adults without high school diploma (%)", y = "Population under 2× FPL (%)", caption  = "Source: OEHHA CalEnviroScreen 4.0")


```
```{r fig-pov-resids}
#| fig-cap: "Residuals vs. fitted values for the simple OLS (poverty ~ education). A random scatter around y=0 supports linearity and homoskedasticity; patterns suggest model violations to assumptions."
...
**Summary**: We see from our results poverty rate tends to be higher in locations where there is a higher percentage of adults with lower levels of education than high school diplomas since the positive slope is statistically significant. If the linear regression assumptions are true, the results we see in @sec-results indicate a moderate effect size in the data that suggest low-education attainment is a relevant predictor when predicting poverty rate (although not the only factor) .
